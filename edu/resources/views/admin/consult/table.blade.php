        ...
        @foreach($consults as $consult)
            <tr class="deleted-row-{{$consult->id}}">
                ...
                <td>
                    ...
                    <a href="{{route('admin.consults.show',$consult->id)}}" data-href="{{route('admin.consults.show',$consult->id)}}" data-entity_id="{{$consult->id}}"  class="btn  btn-sm btn-clean btn-icon" data-toggle="tooltip" title="show">
                        {{ Metronic::getSVG("media/svg/icons/General/Expand-arrows.svg", "svg-icon-md svg-icon-primary") }}

                    </a>
                    <a href="javascript:;" data-href="{{route('admin.consults.catDelete',$consult->id)}}" data-entity_id="{{$consult->id}}" data-token="{{csrf_token()}}" class="btn delete-btn btn-sm btn-clean btn-icon" data-toggle="tooltip" title="حدف">

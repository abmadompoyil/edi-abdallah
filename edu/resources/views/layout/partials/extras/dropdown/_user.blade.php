    ...
    <div class="d-flex align-items-center p-8 rounded-top">
        ...
        <span class="label label-light-success label-lg font-weight-bold label-inline">
            @if(Auth::guard('web')->check())
                مرحبا {{Auth::guard('web')->user()->name}}
            @elseif(Auth::guard('drivers-web')->check())
                {{Auth::user()->property->count()}} عقار
            @endif
...
<div class="navi navi-spacer-x-0 pt-5">
    ...
    <div class="navi-footer  px-8 py-5">
        <a href="{{ route('logout') }}"  class="btn btn-light-primary font-weight-bold" onclick="event.preventDefault();document.getElementById('frm-logout').submit();" > Logout</a>
